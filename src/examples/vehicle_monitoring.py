    ...
    try:
        ...
        for i in range(0, num_vehicle):
            world.try_spawn_actor(random.choice(vehicle_blueprints),
                                  random.choice(vehicle_spawn_points[3:]))
        ...
        some_spawn_point = vehicle_spawn_points[1]
        some_other_spawn_point = vehicle_spawn_points[2]
        # 从蓝图库中挑选我们需要的主车蓝图
        ego_bp = world.get_blueprint_library().find('vehicle.carlamotors.firetruck')
        # 设置主车蓝图的属性中的角色名
        ...
        # 救护车生成位置
        ambulance_spawn_point = some_spawn_point  # 根据实际情况设置
        ambulance_bp = world.get_blueprint_library().find('vehicle.carlamotors.firetruck')
        ambulance_bp.set_attribute('role_name', 'ambulance')

        # 生成救护车
        ambulance_vehicle = world.spawn_actor(ambulance_bp, ambulance_spawn_point)
        ambulance_vehicle.set_autopilot()

        # 生成救护车的RGB相机并绑定
        ambulance_camera_bp = world.get_blueprint_library().find('sensor.camera.rgb')
        ambulance_camera_transform = carla.Transform(carla.Location(x=-8, y=0, z=5),
                                                     carla.Rotation(pitch=10, yaw=0, roll=0))
        ambulance_camera = world.spawn_actor(ambulance_camera_bp, ambulance_camera_transform,
                                             attach_to=ambulance_vehicle,
                                             attachment_type=carla.libcarla.AttachmentType.SpringArmGhost)
        # 警车生成位置
        police_spawn_point = some_other_spawn_point  # 根据实际情况设置
        police_bp = world.get_blueprint_library().find('vehicle.carlamotors.firetruck')
        police_bp.set_attribute('role_name', 'police')
        # 生成警车
        police_vehicle = world.spawn_actor(police_bp, police_spawn_point)
        police_vehicle.set_autopilot()

        # 生成警车的RGB相机并绑定
        police_camera_bp = world.get_blueprint_library().find('sensor.camera.rgb')
        police_camera_transform = carla.Transform(carla.Location(x=-8, y=0, z=5),
                                                  carla.Rotation(pitch=10, yaw=0, roll=0))
        ...
        current_camera_index = 0  # 当前观察的摄像头索引
        # 初始化相机切换时间和间隔
        camera_switch_time = time.time()
        camera_switch_interval = 10  # 切换间隔为 7 秒
        while True:
            # 三个摄像头的位置信息列表，假设已经设置好
            camera_Transforms = [camera.get_transform(), ambulance_camera.get_transform(),
                                 police_camera.get_transform()]
            # 切换观察者位置到当前摄像头
            ...
            # 判断是否到达切换时间间隔
            if current_time - camera_switch_time >= camera_switch_interval:
                # 切换到下一个摄像头
                current_camera_index = (current_camera_index + 1) % len(camera_Transforms)
                # 更新切换时间
                camera_switch_time = current_time
